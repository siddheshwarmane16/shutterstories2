      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 relative z-10">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border border-dashed border-[var(--border-color)] rounded-sm bg-[var(--beige)]/20">
            <p className="text-[var(--secondary-text)]/60 text-xs uppercase tracking-[0.15em]">No testimonial entries found</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-24">
            {reviews.map((rev) => (
              <div key={rev.id} className="bg-[var(--beige)]/45 backdrop-blur-md border border-[var(--border-color)] p-8 sm:p-10 rounded-sm flex flex-col justify-between hover:border-[#C8A96B]/30 hover:shadow-md transition-all duration-500">
                ...
                <div>
                  ...
                  <p className="text-[var(--foreground)]/85 text-xs italic leading-relaxed mb-8 font-light">
                    "{rev.reviewText}"
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4 border-t border-[var(--border-color)] pt-6 mt-6">
                  ...
                  />
                  <div>
                    <h4 className="text-xs uppercase tracking-wider font-bold text-[var(--foreground)]">{rev.clientName}</h4>
        ...
        {/* CTA section */}
        <div className="bg-[var(--beige)]/60 border border-[#C8A96B]/25 p-12 text-center rounded-sm max-w-4xl mx-auto flex flex-col items-center shadow-sm">
          <Sparkles className="h-8 w-8 text-[#C8A96B] mb-6 animate-pulse" />
          <h2 className="font-editorial text-3xl font-bold uppercase mb-4 text-[var(--foreground)]">Be Our Next Story</h2>
          <p className="text-[var(--secondary-text)] text-xs tracking-wide max-w-md mb-8 leading-relaxed">
            Let’s write your cinematic wedding story together. Reach out to coordinate dates.
          </p>
          <Link
            href="/book"
            className="text-xs uppercase tracking-[0.2em] bg-[var(--foreground)] text-[var(--background)] font-bold py-4 px-8 rounded-sm hover:bg-[#C8A96B] hover:text-black transition-all flex items-center gap-2 group border border-transparent hover:border-[#C8A96B]"
          >
